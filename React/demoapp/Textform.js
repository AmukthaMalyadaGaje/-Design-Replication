...
export default function Textform() {
  const bodystyl={
    ...
    let RemoveExtraSpaces=()=>{
      let midtext=text.split(/\s+/).join('');

      setText(midtext.split(/\n/).join(''));
    }
  return (
    <div style={bodystyl}>
    <div style={divStyle} class="d-flex flex-column justify-content-center">
      <label  for="exampleFormControlTextarea1" class="form-label text-white fw-bold fs-4" >Enter Text to Analyze</label>
      ...
      <div style={btnStyle} class="d-flex justify-content-center">
        <button type="button" onClick={handleUpperCase} class="btn btn-primary mx-3">ConvertToUpperCase</button>
        <button type="button" onClick={handleLowerCase} class="btn btn-primary mx-3">ConvertTolowerCase</button>
        <button type="button" onClick={handleCopyText} class="btn btn-primary mx-3">CopyText</button>
        <button type="button" onClick={handleClearText} class="btn btn-primary mx-3">ClearText</button>
        <button type="button" onClick={RemoveExtraSpaces} class="btn btn-primary mx-3">RemoveExtraSpaces</button>
      </div>
    </div>
